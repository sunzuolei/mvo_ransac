...
vector<double> estimateMotion (vector <p_match> p_matched, double K_data[9],parameters param) {
  ...
  /*for (int32_t i=0; i<(int32_t)p_matched.size(); i++)
  {
    ...
    vector<int32_t> inliers_curr = getInlier(p_matched_normalized,F, param);
  ...
  std::cout<<"better matches number with RANSAC="<<inliers.size()<<endl;
 ...
 /* X = X/X.getMat(3,0,3,-1);
  ...
  { 	
      ...
      if (X.val[2][i]>0&&p_matched[i].u1c>200&&p_matched[i].u1c<800&&p_matched[i].v1c>300)
      {
	pos_idx.push_back(i);
  ...
  if(pos_idx.size()<10)
  {
    ...
    //if (X.val[2][i]>0)
    { 
         if (X.val[2][i]>0&&p_matched[i].u1c>200)
        pos_idx.push_back(i);
 ...
 if(pos_idx.size()<10)
  ...
  /*for (int32_t i=0; i<x_plane.n; i++) {
    if (d.val[0][i]>median/param.motion_threshold) {
     ...
     // Find the best plane
   /*for (int i = 0; i < x_plane.n; i++)
    {
        double sumGauss = 0, sumExp = 0, sumTest = 0;
        if (d.val[0][i] > median / param.motion_threshold)
        {
            for (int j = 0; j < x_plane.n; j++)
            {
                double dist = d.val[0][j] - d.val[0][i];
                sumGauss += exp(-dist * dist / 2.0 / (median / 50.0) / (median / 50.0));
                if (dist < 0) sumExp += exp(dist / (median / 50.0));
                if (dist < 0) sumTest += exp(-dist * dist / 2.0 / (median / 50) / (median / 50));
                else          sumTest += exp(-dist * dist / 2.0 / (median / 500) / (median / 500));
            }
            if (sumExp > best_sum)
            {
                best_sum = sumExp;
  ...
  //vector<double> tr=transformationVectorToMatrix(tr_delta);
  return tr_delta;
}
...
Matrix smallerThanMedian (Matrix &X,double &median) {
  ...
  // get median
  int32_t num_elem_half = idx.size()/3;
